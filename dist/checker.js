...
async function checker(service, options) {
    const health = new health_client_1.HealthClient(options.address, options.credentials);
    try {
        const { status } = await health.check(service);
        logger_1.default.verbose(`@fonoster/grpc-health-check healthcheck success [status: ${status}]`);
        process.exit(0);
    }
    catch (error) {
